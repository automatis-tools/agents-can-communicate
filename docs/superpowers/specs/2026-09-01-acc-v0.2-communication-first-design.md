...
`handoff` is null for every other kind. For a handoff it contains the structured `status`,
`completed`, `remaining`, `blockers`, and `verification` fields; the human-readable `body`
remains a deterministic projection of that payload rather than its only representation.

`toParticipantIds` is non-empty for addressed communication. An empty list means a
workspace-room record with obligation `none`. At commit time, core resolves every known
peer participant that currently has an open session and creates one queued receipt per
resolved participant. This gives already-present peers the normal inbox and next-turn
path without changing the durable room audience into an addressed message. Participants
that first appear later can recover room history through a full sync, but do not receive a
retroactive receipt. Room records are never eligible for live push. Only `note`,
`decision`, and `handoff` may be room records.
...
Each addressed or room-resolved `message + recipient` pair has an independent receipt:
...
`livePolicy` is the recipient's explicit `off`, `actionable`, or `all` choice for this
session generation. The adapter obtains it from user-owned client wiring created or
updated by `acc install`; core never infers opt-in from capability or reachability.
