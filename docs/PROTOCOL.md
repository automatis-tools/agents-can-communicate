...
recipient's next turn: the body in the usual untrusted block, after new messages, with the
...
is an attempt, never a transition and never a second message. Each receipt is repeated at
most once, and a receipt that was retrieved, acknowledged or repeated in the meantime is left
unchanged with nothing appended.
