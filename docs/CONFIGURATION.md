...
| `ACC_ANTIGRAVITY_HOOKS` | Where `acc install` registers Antigravity CLI hooks: `global` writes `~/.gemini/config/hooks.json`, which always loads and applies to every Antigravity session on the machine; `workspace` writes `<project>/.agents/hooks.json`, which is scoped to one project and loads only while that project is an open Antigravity workspace. There is no default, and an install that was not told skips that client and says so |
