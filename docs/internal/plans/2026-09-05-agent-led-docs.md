...
- Create test: `tests/docs/links.test.mjs` for local Markdown fragment targets; packed file-target coverage already exists and must remain.
...
- [ ] Add a focused fragment-link gate for README, SECURITY, AGENTS, immediate public `docs/*.md` and `examples/*.md`. Parse local Markdown links (including same-page fragments), derive heading anchors while ignoring fenced code and handling repeated headings, and reject fragments missing in their target document. Test parser cases for fenced sample headings, inline-code headings, duplicate heading suffixes, and encoded fragments. Do not fetch remote URLs. First run it against the existing adapter-authoring links `PROTOCOL.md#identity-hierarchy`, `PROTOCOL.md#delivery-lifecycle`, `CAPABILITIES.md#matrix`, and `CAPABILITIES.md#response-contracts-which-do-not-port`; these currently name nonexistent headings. Then correct the links. Prove the gate by changing one corrected link back to its old broken fragment, observe failure, restore it and pass. Record exact commands and output; use apply_patch for mutations.
- [ ] Run `node --test tests/docs/commands.test.mjs tests/docs/addresses.test.mjs tests/docs/executable.test.mjs tests/docs/skills.test.mjs tests/docs/diagrams.test.mjs tests/docs/links.test.mjs tests/process/surface-coverage.test.mjs tests/process/session-resolution.test.mjs tests/acceptance/package-links.test.mjs`, `git diff --check`, and the full `npm test` once. The stale candidate remains an expected unresolved finalization step, never a skipped gate.
